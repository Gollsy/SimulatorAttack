...
class SWITCH(object):
    ...
    def attack_images(self, batch_index, images, true_labels, target_labels, target_model, surrogate_model, other_surrogate_models, args):
        ...
        for step_index in range(args.max_queries):
            ...
            grad = torch.zeros_like(surrogate_gradients).cuda()
            if idx_improved.sum().item() > 0:
                grad[idx_improved.byte()] = surrogate_gradients[idx_improved.byte()]
            if (1-idx_improved).sum().item() > 0:
                attempt_images_ = image_step(adv_images, -surrogate_gradients, args.image_lr)
                with torch.no_grad():
                    attempt_logits = target_model(attempt_images_)
                attempt_loss_ = criterion(attempt_logits, true_labels, target_labels)
                idx_improved_ = (attempt_loss_ >= l).float()
                if idx_improved_.sum().item() > 0:
                    grad[~idx_improved.byte() & idx_improved_.byte()] = -surrogate_gradients[~idx_improved.byte() & idx_improved_.byte()]
                if (1-idx_improved_).sum().item() > 0:
                    other_surrogate_model = random.choice(other_surrogate_models)
                    other_surrogate_model = other_surrogate_model.cuda()
                    other_surrogate_gradients = self.get_grad(other_surrogate_model, criterion, adv_images, true_labels, target_labels)
                    other_surrogate_model.cpu()
                    grad[~idx_improved.byte() & ~idx_improved_.byte()] = other_surrogate_gradients[~idx_improved.byte() & ~idx_improved_.byte()]
...
def get_exp_dir_name(dataset, loss, norm, targeted, target_type, args):
    ...
    if args.attack_defense:
        dirname = 'SWITCH_neg_rnd_on_defensive_model-{}-{}_loss-{}-{}'.format(dataset, loss, norm, target_str)
    else:
        dirname = 'SWITCH_neg_rnd-{}-{}_loss-{}-{}'.format(dataset, loss, norm, target_str)
    return dirname
...
if __name__ == "__main__":
    ...
    attacker = SWITCH(args.dataset, args.batch_size, args.targeted, args.target_type, args.epsilon,
