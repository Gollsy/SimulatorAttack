...
def get_exp_dir_name(dataset, lr, loss, norm, targeted, target_type, args):
    ...
    if args.NO_SWITCH:
        if args.attack_defense:
            ...
            dirname = 'NO_SWITCH-{}-{}_lr_{:.3f}-loss-{}-{}'.format(dataset, loss, lr, norm, target_str)
    else:
        if args.attack_defense:
            dirname = 'SWITCH_neg_save_on_defensive_model-{}-{}_lr_{}-loss-{}-{}'.format(dataset, loss, lr, norm, target_str)
        else:
            dirname = 'SWITCH_neg_save-{}-{}_lr_{}-loss-{}-{}'.format(dataset, loss, lr, norm, target_str)
    return dirname
