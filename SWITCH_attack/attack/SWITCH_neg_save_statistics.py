...
def get_exp_dir_name(dataset, loss, norm, targeted, target_type, args):
    ...
    if args.NO_SWITCH:
        if args.attack_defense:
            dirname = 'NO_SWITCH_neg_save_stats_attack_on_defensive_model-{}-{}_loss-{}-{}'.format(dataset, loss, norm,
                                                                                             target_str)
        else:
            dirname = 'NO_SWITCH_neg_save_stats_attack-{}-{}_loss-{}-{}'.format(dataset, loss, norm, target_str)
    else:
        if args.attack_defense:
