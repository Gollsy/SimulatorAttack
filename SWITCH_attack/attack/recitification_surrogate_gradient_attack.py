...
import argparse
from SWITCH_attack.attack.group_generator import EquallySplitGrouping
...
class NESRecitificationSurrogateGradientAttack(object):
    def __init__(self, args, dataset, batch_size, targeted, target_type, epsilon, norm, lower_bound=0.0, upper_bound=1.0,
                 max_queries=10000):
    ...
    def get_true_gradient(self, model, loss_fn, x, true_labels, target_labels):
    ...
    # TODO 修改为batch模式
    def estimate_rectified_gradient(self, model, adv_images, surrogate_gradient, n, sigma,  true_labels, target_labels,
                                    sub_num, group_gen, loss_fn):
        # 此处的frame_number只能选择1
        if target_labels is not None:
            target_class = target_labels[0].item()
        else:
            target_class = true_labels[0].item()
        with torch.no_grad():
            ...
            for _ in range(n // sub_num):
                ...
                noise_list = torch.randn((sub_num // 2,) + grads.size()).cuda() * sigma  # shape = (sub_num//2, frame_number * patch number * patch_number)
                all_noise = torch.cat([noise_list, -noise_list], 0)   # shape = (sub_num, frame_number * patch number * patch_number)
                perturbation_sample = group_gen.apply_group_change(surrogate_gradient, all_noise)  # 1 个patch一个扰动数值
                ...
                if target_labels is not None:
                    target_labels = target_labels.repeat(sub_num)
                loss = loss_fn(logits, true_labels.repeat(sub_num), target_labels)
                # if self.targeted:
            ...
            if self.rank_transform:
                ...
                unavailable_weight = torch.sum(torch.where(good_idx, torch.tensor(0., device='cuda'),
                                    loss_order)) / unavailable_number if unavailable_number else torch.tensor(0., device='cuda')
    ...
    def attack_images(self, batch_index, images, true_labels, target_labels, target_model, surrogate_model, args):
        ...
        while query.min().item() < args.max_queries:
            ...
            surrogate_gradients = self.get_true_gradient(surrogate_model, criterion, adv_images, true_labels, target_labels)
            group_gen.initialize(surrogate_gradients)
            ...
            l, g = self.estimate_rectified_gradient(target_model, adv_images, surrogate_gradients, self.sample_per_draw,
                                                    self.sigma, true_labels, target_labels, self.sub_num_sample, group_gen, criterion)
            query = query + not_done
            query = query + (1 - idx_improved) * not_done * self.sample_per_draw
            if l is None and g is None:
                log.info('NES fails, try again....')
                continue
            assert g.size(0) == len(group_gen), 'rectification vector size error!'
            rectified_directions = group_gen.apply_group_change(surrogate_gradients,
                                                                torch.sign(g) if self.norm == "linf" else g)
            idx_improved = idx_improved.view(-1, 1, 1, 1)
            grad = surrogate_gradients * idx_improved + (1-idx_improved) * rectified_directions
...
def get_exp_dir_name(dataset, loss, norm, targeted, target_type, args):
    ...
    if args.attack_defense:
        ...
        dirname = 'recitification_surrogate_gradient_attack-{}-{}_loss-{}-{}'.format(dataset, loss, norm, target_str)
    return dirname
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch-size', type=int, default=1, help='The mini-batch size')
    ...
    attacker = NESRecitificationSurrogateGradientAttack(args, args.dataset, args.batch_size, args.targeted, args.target_type, args.epsilon,
