        ...
        for data_idx, data_tuple in enumerate(self.dataset_loader):
            ...
            for step_index in range(1, args.max_queries + 1):
                ...
                if (step_index <= args.warm_up_steps or (
                    ...
                    finetune_queue.append(q1_images.detach(), q2_images.detach(), q1_logits.detach(),
                                          q2_logits.detach())
                ...
                log.info('       not_done: {:.4f}'.format(
                    len(np.where(not_done.detach().cpu().numpy().astype(np.int32) == 1)[0]) / float(args.batch_size)))
...
def get_exp_dir_name(meta_mode, dataset, loss, norm, targeted, target_type, distillation_loss, args):
    target_str = "untargeted" if not targeted else "targeted_{}".format(target_type)
    dirname = 'simulate_bandits_on_{}-{}-{}_loss-{}-{}-{}'.format(meta_mode, dataset, loss, norm, target_str, distillation_loss)
    return dirname
...
def attack_dataset(args, gpu, save_result_path, log_file_path):
    ...
    tmp_result_path = os.path.dirname(save_result_path) + "/" + "tmp_"+os.path.basename(save_result_path)
    log.info("tmp path is {}".format(tmp_result_path))
    attacker.attack_all_images(args, args.arch, tmp_result_path, save_result_path)
    log.info("Done!")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--meta_mode",type=str)
    parser.add_argument("--study_subject", type=str, default="meta_or_not")   # logits error 和 loss error 这两种模式在meta_or_not中
    ...
    meta_modes = ["random_init"]  # FIXME
    args.exp_dir = osp.join(args.exp_dir, get_exp_dir_name(meta_modes[0], args.dataset, args.data_loss,
