...
class GradCAM(object):
    ...
    def __init__(self, model_dict, verbose=False):
        ...
        elif model_type.lower() in MODELS_TEST_STANDARD["ImageNet"]:
            target_layer = find_imagenet_layer(self.model_arch, layer_name)
    ...
    def forward(self, input, class_idx=None, retain_graph=False):
        ...
        saliency_map = (saliency_map - saliency_map_min).div(saliency_map_max - saliency_map_min) # 1,1,H,W
        return saliency_map
...
def test_grad_cam(folders):
    ...
    for img_file_name in os.listdir(folders):
        ...
        image = cv2.imread(img_file_name, cv2.IMREAD_COLOR)
        image = image.astype(np.float32)#H,W,C
        image = np.transpose(image, (2,0,1)) # C,H,W
        image = torch.from_numpy(image).unsqueeze(0).cuda()
        saliency_map = cam.forward(image, class_id)
        heatmap, _ = cam.visualize_cam(saliency_map, image)
