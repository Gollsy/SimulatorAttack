...
from config import PY_ROOT, MODELS_TEST_STANDARD, CLASS_NUM, MODELS_TRAIN_WITHOUT_RESNET, MODELS_TRAIN_STANDARD
...
class SquareAttack(object):
    ...
    def get_grad(self, model, x, y, loss_type):
        with torch.enable_grad():
            x.requires_grad_()
            logits = model(x)
            loss = self.loss(logits, y, loss_type)
            gradient = torch.autograd.grad(loss.mean(), x, torch.ones_like(loss), retain_graph=False)[0].detach().cpu().numpy()
        return loss.detach().cpu().numpy(), gradient
    ...
    def square_attack_l2(self, model, surrogate_models, x, y, eps, max_queries, p_init, loss_type):
        ...
        for i_iter in range(n_iters):
            ...
            proj_step = proj_maker(torch.from_numpy(x_curr).float(), eps)
            ...
            surrogate_loss_before, gradient = self.get_grad(surrogate_model,
                                                            torch.from_numpy(x_best_curr).cuda().float(),
                                                            torch.from_numpy(y_curr).cuda().long(),
                                                            loss_type)
            adv_images = image_step(torch.from_numpy(x_best_curr).float(), torch.from_numpy(gradient).float(),
                                    eps)  # prior放大后相当于累积的更新量，可以用来更新
            adv_images = proj_step(adv_images)
            surrogate_delta = adv_images.detach().cpu().numpy().astype(x_curr.dtype) - x_curr
            ...
            delta_curr[:, :, center_h:center_h + s, center_w:center_w + s] = surrogate_delta[:, :, center_h:center_h + s, center_w:center_w + s]
    ...
    def modify_array(self, array, idx_to_fool, idx_improved_surrogate):
        array_to_fool = array[idx_to_fool]
        array_to_fool[idx_improved_surrogate] += 1
        array[idx_to_fool] = array_to_fool

    def square_attack_linf(self, model, surrogate_models, x, y, eps, max_queries, p_init, loss_type):
        ...
        for i_iter in range(n_iters - 1):
            ...
            surrogate_model = surrogate_model.cuda()
            surrogate_loss_before, gradient = self.get_grad(surrogate_model,
                                     torch.from_numpy(x_best_curr).cuda().float(), torch.from_numpy(y_curr).long().cuda(),
                                     loss_type)
            adv_images = image_step(torch.from_numpy(x_best_curr).float(), torch.from_numpy(gradient).float(), eps)  # prior放大后相当于累积的更新量，可以用来更新
            adv_images = proj_step(adv_images)
            surrogate_delta = adv_images.detach().cpu().numpy().astype(x_curr.dtype) - x_curr
            ...
            for i_img in range(x_best_curr.shape[0]):
                ...
                while np.sum(np.abs(np.clip(x_curr_window + deltas[i_img, :, center_h:center_h + s, center_w:center_w + s],
                    ...
                    deltas[i_img, :, center_h:center_h + s, center_w:center_w + s] = surrogate_delta[i_img, :, center_h:center_h+s, center_w:center_w+s]

            x_new = np.clip(x_curr + deltas, self.lower_bound, self.upper_bound)
            # with torch.no_grad():
            #     surrogate_logits = surrogate_model(torch.from_numpy(x_new).cuda().float())
            #     surrogate_loss_after = self.loss(surrogate_logits, torch.from_numpy(y_curr).long().cuda(), loss_type=loss_type).detach().cpu().numpy()
            #     idx_improved_surrogate = surrogate_loss_after < surrogate_loss_before   # loss变小了，可以去查询目标模型，否则直接拒绝，不要查询目标模型
            surrogate_model.cpu()
            ...
            n_queries[idx_to_fool] += 1
            # n_queries_to_fool = n_queries[idx_to_fool]
            # n_queries_to_fool[idx_improved_surrogate] += 1
            # n_queries[idx_to_fool] = n_queries_to_fool
            acc = (margin_min > 0.0).sum() / n_ex_total
            acc_corr = (margin_min > 0.0).mean()
    ...
    def attack_all_images(self, args, arch_name, target_model, surrogate_models, result_dump_path):
        ...
        for batch_idx, data_tuple in enumerate(self.dataset_loader):
            ...
            if self.norm == "l2":
                query, adv_images = self.square_attack_l2(target_model, surrogate_models, images.detach().cpu().numpy(),
                                                          labels.detach().cpu().numpy(),
                                         args.epsilon, args.max_queries, args.p, loss_type)
            elif self.norm == "linf":
                query, adv_images = self.square_attack_linf(target_model, surrogate_models, images.detach().cpu().numpy(),
                                                            labels.detach().cpu().numpy(),
                                                            args.epsilon, args.max_queries, args.p, loss_type)
...
def get_exp_dir_name(dataset, norm, targeted, target_type, args):
    ...
    if args.attack_defense:
        dirname = 'assistant_square_attack_on_defensive_model-{}-{}-{}'.format(dataset, norm, target_str)
    else:
        dirname = 'assistant_square_attack-{}-{}-{}'.format(dataset, norm, target_str)
    return dirname
...
def main():
    parser = argparse.ArgumentParser(description='Assistant Square Attack Hyperparameters.')
