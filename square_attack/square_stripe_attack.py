...
class SquareStripeAttack(object):
    def __init__(self, dataset, batch_size, targeted, target_type, update_stripe,
                 epsilon, norm, lower_bound=0.0, upper_bound=1.0,
                 max_queries=10000):
        """
        ...
        self.update_stripe = update_stripe
        ...
        for i_iter in range(n_iters - 1):
            ...
            for i_img in range(x_best_curr.shape[0]):
                ...
                while np.sum(np.abs(np.clip(x_curr_window + deltas[i_img, :, center_h:center_h + s, center_w:center_w + s], self.lower_bound,
                    ...
                    if self.update_stripe == "horizontal_stripe":
                        stripe = np.zeros(shape=(c,s,1))
                        start_from = random.randint(0,1)
                        stripe[:,start_from::2, :] = np.random.choice([-eps, eps], size=[c, 1, 1])
                    elif self.update_stripe == "vertical_stripe":
                        stripe = np.zeros(shape=(c, 1, s))
                        start_from = random.randint(0,1)
                        stripe[:, :, start_from::2] = np.random.choice([-eps, eps], size=[c, 1, 1])

                    deltas[i_img, :, center_h:center_h + s, center_w:center_w + s] = stripe
...
def get_exp_dir_name(dataset, norm, targeted, target_type, args):
    ...
    if args.attack_defense:
        dirname = 'square_stripe_attack_on_defensive_model-{}-{}-{}'.format(dataset, norm, target_str)
    else:
        dirname = 'square_stripe_attack-{}-{}-{}'.format(dataset, norm, target_str)
    return dirname
...
def main():
    ...
    parser.add_argument('--stripe', type=str, required=True,choices=["horizontal_stripe","vertical_stripe"])
    ...
    if args.test_archs:
        if args.attack_defense:
            log_file_path = osp.join(args.exp_dir, 'run_defense_{}_{}.log'.format(args.defense_model, args.stripe))
        else:
            log_file_path = osp.join(args.exp_dir, 'run_{}.log'.format(args.stripe))
    elif args.arch is not None:
        if args.attack_defense:
            log_file_path = osp.join(args.exp_dir, 'run_defense_{}_{}_{}.log'.format(args.arch, args.defense_model, args.stripe))
        else:
            log_file_path = osp.join(args.exp_dir, 'run_{}_{}.log'.format(args.arch, args.stripe))
    ...
    print_args(args)
    attacker = SquareStripeAttack(args.dataset, args.batch_size,
                                  args.targeted, args.target_type, args.stripe, args.epsilon, args.norm, max_queries=args.max_queries)
    for arch in archs:
        if args.attack_defense:
            save_result_path = args.exp_dir + "/{}_{}_{}_result.json".format(arch, args.defense_model, args.stripe)
        else:
            save_result_path = args.exp_dir + "/{}_{}_result.json".format(arch, args.stripe)
