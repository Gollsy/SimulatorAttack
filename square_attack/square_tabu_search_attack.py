...
from config import PY_ROOT, MODELS_TEST_STANDARD, CLASS_NUM, IN_CHANNELS, IMAGE_SIZE
...
class SquareTabuListAttack(object):
    def __init__(self, dataset, batch_size, targeted, target_type, epsilon, norm, lower_bound=0.0, upper_bound=1.0,
        ...
        # the count of pixels
        # FIXME ImageNet的IMAGE_SIZE和实际图不符
    ...
    def square_attack_l2(self, model, x, y, eps, max_queries, p_init, loss_type):
        ...
        self.tabu_list = [faiss.IndexFlatL2(np.prod(x.shape[1:]).item()) for _ in range(x.shape[0])]
        ...
        for counter in range(h // s):
            ...
            for counter2 in range(w // s):
                ...
                center_w += s
            center_h += s
        for batch_id, delta in enumerate(delta_init):
            self.tabu_list[batch_id].add(delta.reshape(-1))
        ...
        for i_iter in range(n_iters):
            ...
            nearest_vectors = []
            deltas_flatten = delta_curr.reshape(delta_curr.shape[0], -1)  # N,D
            filtered_tabu_list = [self.tabu_list[ind] for ind, ib in enumerate(idx_to_fool) if ib == True]
            for index, tabu in enumerate(filtered_tabu_list):
                D, I = tabu.search(deltas_flatten[index], 1)
                nearest_vectors.append(I[0])
            nearest_vectors = np.stack(nearest_vectors)  # N,D
            diff_norm = np.linalg.norm(deltas_flatten - nearest_vectors, axis=1, keepdims=False, ord=1)  # shape = N
            good_indexes = (diff_norm > self.threshold_diff).astype(np.bool)  # True: can be queried, False: has queried before
            all_good_indexes = np.zeros(x.shape[0]).astype(np.bool)
            all_good_indexes[idx_to_fool] = good_indexes
            for index, delta in enumerate(deltas_flatten):
                filtered_tabu_list[index].add(delta)
            ...
            n_queries[idx_to_fool & all_good_indexes] += 1
    ...
    def square_attack_linf(self, model, x, y, eps, max_queries, p_init, loss_type):
        ...
        self.tabu_list = [faiss.IndexFlatL2(np.prod(x.shape[1:]).item()) for _ in range(x.shape[0])]
        init_delta = np.random.choice([-eps, eps], size=[x.shape[0], c, 1, w]).repeat(h, axis=2)
        for batch_id, delta in enumerate(init_delta):
            self.tabu_list[batch_id].add(delta.reshape(-1))
        ...
        for i_iter in range(n_iters - 1):
            ...
            for i_img in range(x_best_curr.shape[0]):
                ...
                while np.sum(np.abs(np.clip(x_curr_window + deltas[i_img, :, center_h:center_h + s, center_w:center_w + s], self.lower_bound,
                    ...
                    deltas[i_img, :, center_h:center_h + s, center_w:center_w + s] = np.random.choice([-eps, eps],
                                                                                                      size=[c, 1, 1])
            nearest_vectors = []
            deltas_flatten = deltas.reshape(deltas.shape[0], -1)  # N,D
            filtered_tabu_list = [self.tabu_list[ind] for ind, ib in enumerate(idx_to_fool) if ib == True]
            for index, tabu in enumerate(filtered_tabu_list):
                D, I = tabu.search(deltas_flatten[index],1)
                nearest_vectors.append(I[0])
            ...
            diff_norm = np.linalg.norm(deltas_flatten - nearest_vectors, axis=1, keepdims=False, ord=1)  # shape = N
            good_indexes = (diff_norm > self.threshold_diff).astype(np.bool)  # True: can be queried, False: has queried before
            # good_indexes 的 shape = idx_fool, 需要扩张
            all_good_indexes = np.zeros(x.shape[0]).astype(np.bool)
            all_good_indexes[idx_to_fool] = good_indexes
            for index, delta in enumerate(deltas_flatten):
                filtered_tabu_list[index].add(delta)
            ...
            idx_improved = np.reshape(idx_improved, [-1, *[1] * len(x.shape[:-1])])
            x_best[idx_to_fool] = idx_improved * x_new + ~idx_improved * x_best_curr
            n_queries[idx_to_fool & all_good_indexes] += 1  #  只有这个地方计数的时候+1
            ...
            #     format(i_iter + 1, acc, acc_corr, mean_nq_ae, median_nq_ae, avg_margin_min, x.shape[0], eps,
            #            time_total))
    ...
    def attack_all_images(self, args, arch_name, target_model, result_dump_path):
        ...
        for batch_idx, data_tuple in enumerate(self.dataset_loader):
            ...
            self.pixel_count = self.lowest_change_ratio * images.size(-3) * images.size(-2) * images.size(-1)

            if self.norm == "l2":
                self.threshold_diff = np.sqrt((self.epsilon ** 2 / (
                            images.size(-3) * images.size(-2) * images.size(-1))) * self.pixel_count).item()
            elif self.norm == "linf":
                self.threshold_diff = self.epsilon * self.pixel_count

            if self.norm == "l2":
                query, adv_images = self.square_attack_l2(target_model, images.detach().cpu().numpy(),
                                                          labels.detach().cpu().numpy(),
                                         args.epsilon, args.max_queries, args.p, loss_type)
            elif self.norm == "linf":
                query, adv_images = self.square_attack_linf(target_model, images.detach().cpu().numpy(),
                                                            labels.detach().cpu().numpy(),
                                                            args.epsilon, args.max_queries, args.p, loss_type)
...
def get_exp_dir_name(dataset, norm, targeted, target_type, args):
    ...
    if args.attack_defense:
        dirname = 'tabu_square_attack_on_defensive_model-{}-{}-{}'.format(dataset, norm, target_str)
    else:
        dirname = 'tabu_square_attack-{}-{}-{}'.format(dataset, norm, target_str)
    return dirname
...
def main():
    parser = argparse.ArgumentParser(description='Tabu Square Attack Hyperparameters.')
    ...
    print_args(args)
    attacker = SquareTabuListAttack(args.dataset, args.batch_size,
