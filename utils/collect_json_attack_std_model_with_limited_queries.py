...
def from_method_to_dir_path(dataset, method, norm, targeted):
    ...
    if method == "bandits_attack":
        ...
        if dataset.startswith("CIFAR"):
            path = "SWITCH_RGF-resnet-110-{dataset}-{loss}-loss-{norm}-{target_str}".format(dataset=dataset,
                                                                                                               loss="cw" if not targeted else "xent",
                                                                                                               norm=norm, target_str="untargeted" if not targeted else "targeted_increment")
        elif dataset == "TinyImageNet":
            path = "SWITCH_RGF-resnet101-{dataset}-{loss}-loss-{norm}-{target_str}".format(
                dataset=dataset,
                loss="cw" if not targeted else "xent",
                norm=norm, target_str="untargeted" if not targeted else "targeted_increment")
    return path
...
def get_file_name_list(dataset, method_name_to_paper, norm, targeted):
    ...
    for method, paper_method_name in method_name_to_paper.items():
        if norm == "l2" and method == "parsimonious_attack":
            continue
        file_path = "/mnt/workspace/query_based_black_box_attack/logs/" + from_method_to_dir_path(dataset, method, norm, targeted)
...
def fetch_all_json_content_given_contraint(dataset, norm, targeted, arch, limited_queries):
    ...
    for method, folder in folder_list.items():
        ...
        # avg_query_over_successful_samples = int(new_round(query_all.mean().item(), 0))
        # median_query_over_successful_samples = int(new_round(np.median(query_all).item(),0))
        if norm == "l2":
            if "CIFAR" in dataset:
                epsilon = 1.0
            else:
                epsilon = 2.0
            assert epsilon == json_content["args"]["epsilon"], "eps is {} in {}".format(json_content["args"]["epsilon"],
                                                                                        file_path)
        else:
            epsilon = 0.031372
...
def draw_tables_for_TinyImageNet(norm, archs_result):
    result = archs_result
    avg_q = "avg_query_over_all_samples"
    med_q = "median_query_over_all_samples"
    if norm == "linf":
        print("""
...
SWITCH & {D121_SWITCH_neg_ASR}\% & {R32_SWITCH_neg_ASR}\% & {R64_SWITCH_neg_ASR}\% & {D121_SWITCH_neg_AVGQ} & {R32_SWITCH_neg_AVGQ} & {R64_SWITCH_neg_AVGQ} & {D121_SWITCH_neg_MEDQ} & {R32_SWITCH_neg_MEDQ} & {R64_SWITCH_neg_MEDQ} \\\\
SWITCH$_\\text{{RGF}}$ & {D121_SWITCH_RGF_ASR}\% & {R32_SWITCH_RGF_ASR}\% & {R64_SWITCH_RGF_ASR}\% & {D121_SWITCH_RGF_AVGQ} & {R32_SWITCH_RGF_AVGQ} & {R64_SWITCH_RGF_AVGQ} & {D121_SWITCH_RGF_MEDQ} & {R32_SWITCH_RGF_MEDQ} & {R64_SWITCH_RGF_MEDQ} \\\\
        """.format(
        ...
        SWITCH$_\\text{{RGF}}$ & {D121_SWITCH_RGF_ASR}\% & {R32_SWITCH_RGF_ASR}\% & {R64_SWITCH_RGF_ASR}\% & {D121_SWITCH_RGF_AVGQ} & {R32_SWITCH_RGF_AVGQ} & {R64_SWITCH_RGF_AVGQ} & {D121_SWITCH_RGF_MEDQ} & {R32_SWITCH_RGF_MEDQ} & {R64_SWITCH_RGF_MEDQ} \\\\
            ...
            D121_NO_SWITCH_ASR=result["densenet121"]["NO_SWITCH"]["success_rate"],
            R32_NO_SWITCH_ASR=result["resnext32_4"]["NO_SWITCH"]["success_rate"],
            R64_NO_SWITCH_ASR=result["resnext64_4"]["NO_SWITCH"]["success_rate"],
            D121_NO_SWITCH_AVGQ=result["densenet121"]["NO_SWITCH"][avg_q],
            R32_NO_SWITCH_AVGQ=result["resnext32_4"]["NO_SWITCH"][avg_q],
            R64_NO_SWITCH_AVGQ=result["resnext64_4"]["NO_SWITCH"][avg_q],
            D121_NO_SWITCH_MEDQ=result["densenet121"]["NO_SWITCH"][med_q],
...
def draw_tables_for_TinyImageNet_median_avg_query(norm, archs_result):
    result = archs_result
    avg_q = "avg_query_over_all_samples"
    med_q = "median_query_over_all_samples"
    if norm == "linf":
        print("""
RGF \cite{{2017RGF}} & {D121_RGF_ASR}\% & {R32_RGF_ASR}\% & {R64_RGF_ASR}\% & {D121_RGF_MEDQ} & {R32_RGF_MEDQ} & {R64_RGF_MEDQ} & {D121_RGF_AVGQ} & {R32_RGF_AVGQ} & {R64_RGF_AVGQ} \\\\
P-RGF \cite{{cheng2019improving}} & {D121_PRGF_ASR}\% & {R32_PRGF_ASR}\% & {R64_PRGF_ASR}\% & {D121_PRGF_MEDQ} & {R32_PRGF_MEDQ} & {R64_PRGF_MEDQ} & {D121_PRGF_AVGQ} & {R32_PRGF_AVGQ} & {R64_PRGF_AVGQ} \\\\
Bandits \cite{{ilyas2018prior}} & {D121_Bandits_ASR}\% & {R32_Bandits_ASR}\% & {R64_Bandits_ASR}\% & {D121_Bandits_MEDQ} & {R32_Bandits_MEDQ} & {R64_Bandits_MEDQ} & {D121_Bandits_AVGQ} & {R32_Bandits_AVGQ} & {R64_Bandits_AVGQ} \\\\
PPBA \cite{{li2020projection}} & {D121_PPBA_ASR}\% & {R32_PPBA_ASR}\% & {R64_PPBA_ASR}\% & {D121_PPBA_MEDQ} & {R32_PPBA_MEDQ} & {R64_PPBA_MEDQ} & {D121_PPBA_AVGQ} & {R32_PPBA_AVGQ} & {R64_PPBA_AVGQ}  \\\\
Parsimonious \cite{{moonICML19}} & {D121_Parsimonious_ASR}\% & {R32_Parsimonious_ASR}\% & {R64_Parsimonious_ASR}\% & {D121_Parsimonious_MEDQ} & {R32_Parsimonious_MEDQ} & {R64_Parsimonious_MEDQ} & {D121_Parsimonious_AVGQ} & {R32_Parsimonious_AVGQ} & {R64_Parsimonious_AVGQ}  \\\\
SignHunter \cite{{al2019sign}} & {D121_SignHunter_ASR}\% & {R32_SignHunter_ASR}\% & {R64_SignHunter_ASR}\% & {D121_SignHunter_MEDQ} & {R32_SignHunter_MEDQ} & {R64_SignHunter_MEDQ}  & {D121_SignHunter_AVGQ} & {R32_SignHunter_AVGQ} & {R64_SignHunter_AVGQ} \\\\
Square Attack \cite{{ACFH2020square}} & {D121_Square_ASR}\% & {R32_Square_ASR}\% & {R64_Square_ASR}\% & {D121_Square_MEDQ} & {R32_Square_MEDQ} & {R64_Square_MEDQ} & {D121_Square_AVGQ} & {R32_Square_AVGQ} & {R64_Square_AVGQ}  \\\\
NO SWITCH & {D121_NO_SWITCH_ASR}\% & {R32_NO_SWITCH_ASR}\% & {R64_NO_SWITCH_ASR}\% & {D121_NO_SWITCH_MEDQ} & {R32_NO_SWITCH_MEDQ} & {R64_NO_SWITCH_MEDQ} & {D121_NO_SWITCH_AVGQ} & {R32_NO_SWITCH_AVGQ} & {R64_NO_SWITCH_AVGQ}  \\\\
SWITCH & {D121_SWITCH_neg_ASR}\% & {R32_SWITCH_neg_ASR}\% & {R64_SWITCH_neg_ASR}\% & {D121_SWITCH_neg_MEDQ} & {R32_SWITCH_neg_MEDQ} & {R64_SWITCH_neg_MEDQ} & {D121_SWITCH_neg_AVGQ} & {R32_SWITCH_neg_AVGQ} & {R64_SWITCH_neg_AVGQ}  \\\\
SWITCH$_\\text{{RGF}}$ & {D121_SWITCH_RGF_ASR}\% & {R32_SWITCH_RGF_ASR}\% & {R64_SWITCH_RGF_ASR}\% & {D121_SWITCH_RGF_MEDQ} & {R32_SWITCH_RGF_MEDQ} & {R64_SWITCH_RGF_MEDQ} & {D121_SWITCH_RGF_AVGQ} & {R32_SWITCH_RGF_AVGQ} & {R64_SWITCH_RGF_AVGQ}  \\\\
        """.format(
            D121_RGF_ASR=result["densenet121"]["RGF"]["success_rate"],R32_RGF_ASR=result["resnext32_4"]["RGF"]["success_rate"],R64_RGF_ASR=result["resnext64_4"]["RGF"]["success_rate"],
            D121_RGF_AVGQ=result["densenet121"]["RGF"][avg_q], R32_RGF_AVGQ=result["resnext32_4"]["RGF"][avg_q], R64_RGF_AVGQ=result["resnext64_4"]["RGF"][avg_q],
            D121_RGF_MEDQ=result["densenet121"]["RGF"][med_q], R32_RGF_MEDQ=result["resnext32_4"]["RGF"][med_q], R64_RGF_MEDQ=result["resnext64_4"]["RGF"][med_q],
...
NO SWITCH & {D121_NO_SWITCH_ASR}\% & {R32_NO_SWITCH_ASR}\% & {R64_NO_SWITCH_ASR}\% & {D121_NO_SWITCH_MEDQ} & {R32_NO_SWITCH_MEDQ} & {R64_NO_SWITCH_MEDQ} & {D121_NO_SWITCH_AVGQ} & {R32_NO_SWITCH_AVGQ} & {R64_NO_SWITCH_AVGQ}  \\\\
SWITCH & {D121_SWITCH_neg_ASR}\% & {R32_SWITCH_neg_ASR}\% & {R64_SWITCH_neg_ASR}\% & {D121_SWITCH_neg_MEDQ} & {R32_SWITCH_neg_MEDQ} & {R64_SWITCH_neg_MEDQ} & {D121_SWITCH_neg_AVGQ} & {R32_SWITCH_neg_AVGQ} & {R64_SWITCH_neg_AVGQ}  \\\\
SWITCH_RGF & {D121_SWITCH_RGF_ASR}\% & {R32_SWITCH_RGF_ASR}\% & {R64_SWITCH_RGF_ASR}\% & {D121_SWITCH_RGF_MEDQ} & {R32_SWITCH_RGF_MEDQ} & {R64_SWITCH_RGF_MEDQ} & {D121_SWITCH_RGF_AVGQ} & {R32_SWITCH_RGF_AVGQ} & {R64_SWITCH_RGF_AVGQ}  \\\\
                """.format(
            D121_RGF_ASR=result["densenet121"]["RGF"]["success_rate"],
            R32_RGF_ASR=result["resnext32_4"]["RGF"]["success_rate"],
            R64_RGF_ASR=result["resnext64_4"]["RGF"]["success_rate"],
...
def draw_tables_for_TinyImageNet_with_slash(norm, result_small_queries, result_big_queries):
    result = result_big_queries
    result_2 = result_small_queries
    avg_q = "avg_query_over_all_samples"
    med_q = "median_query_over_all_samples"
    if norm == "linf":
        print("""
RGF & {D121_RGF_ASR}\%/\\textcolor{{magenta}}{{{D121_RGF_ASR_small}\%}} & {R32_RGF_ASR}\%/\\textcolor{{magenta}}{{{R32_RGF_ASR_small}\%}} & {R64_RGF_ASR}\%/\\textcolor{{magenta}}{{{R64_RGF_ASR_small}\%}} & {D121_RGF_AVGQ} & {R32_RGF_AVGQ} & {R64_RGF_AVGQ} & {D121_RGF_MEDQ} & {R32_RGF_MEDQ} & {R64_RGF_MEDQ} \\\\
P-RGF & {D121_PRGF_ASR}\%/\\textcolor{{magenta}}{{{D121_PRGF_ASR_small}\%}} & {R32_PRGF_ASR}\%/\\textcolor{{magenta}}{{{R32_PRGF_ASR_small}\%}} & {R64_PRGF_ASR}\%/\\textcolor{{magenta}}{{{R64_PRGF_ASR_small}\%}} & {D121_PRGF_AVGQ} & {R32_PRGF_AVGQ} & {R64_PRGF_AVGQ} & {D121_PRGF_MEDQ} & {R32_PRGF_MEDQ} & {R64_PRGF_MEDQ} \\\\
Bandits & {D121_Bandits_ASR}\%/\\textcolor{{magenta}}{{{D121_Bandits_ASR_small}\%}} & {R32_Bandits_ASR}\%/\\textcolor{{magenta}}{{{R32_Bandits_ASR_small}\%}} & {R64_Bandits_ASR}\%/\\textcolor{{magenta}}{{{R64_Bandits_ASR_small}\%}} & {D121_Bandits_AVGQ} & {R32_Bandits_AVGQ} & {R64_Bandits_AVGQ} & {D121_Bandits_MEDQ} & {R32_Bandits_MEDQ} & {R64_Bandits_MEDQ} \\\\
PPBA & {D121_PPBA_ASR}\%/\\textcolor{{magenta}}{{{D121_PPBA_ASR_small}\%}} & {R32_PPBA_ASR}\%/\\textcolor{{magenta}}{{{R32_PPBA_ASR_small}\%}} & {R64_PPBA_ASR}\%/\\textcolor{{magenta}}{{{R64_PPBA_ASR_small}\%}} & {D121_PPBA_AVGQ} & {R32_PPBA_AVGQ} & {R64_PPBA_AVGQ} & {D121_PPBA_MEDQ} & {R32_PPBA_MEDQ} & {R64_PPBA_MEDQ} \\\\
Parsimonious & {D121_Parsimonious_ASR}\%/\\textcolor{{magenta}}{{{D121_Parsimonious_ASR_small}\%}} & {R32_Parsimonious_ASR}\%/\\textcolor{{magenta}}{{{R32_Parsimonious_ASR_small}\%}} & {R64_Parsimonious_ASR}\%/\\textcolor{{magenta}}{{{R64_Parsimonious_ASR_small}\%}} & {D121_Parsimonious_AVGQ} & {R32_Parsimonious_AVGQ} & {R64_Parsimonious_AVGQ} & {D121_Parsimonious_MEDQ} & {R32_Parsimonious_MEDQ} & {R64_Parsimonious_MEDQ} \\\\
SignHunter & {D121_SignHunter_ASR}\%/\\textcolor{{magenta}}{{{D121_SignHunter_ASR_small}\%}} & {R32_SignHunter_ASR}\%/\\textcolor{{magenta}}{{{R32_SignHunter_ASR_small}\%}} & {R64_SignHunter_ASR}\%/\\textcolor{{magenta}}{{{R64_SignHunter_ASR_small}\%}} & {D121_SignHunter_AVGQ} & {R32_SignHunter_AVGQ} & {R64_SignHunter_AVGQ} & {D121_SignHunter_MEDQ} & {R32_SignHunter_MEDQ} & {R64_SignHunter_MEDQ} \\\\
Square Attack & {D121_Square_ASR}\%/\\textcolor{{magenta}}{{{D121_Square_ASR_small}\%}} & {R32_Square_ASR}\%/\\textcolor{{magenta}}{{{R32_Square_ASR_small}\%}} & {R64_Square_ASR}\%/\\textcolor{{magenta}}{{{R64_Square_ASR_small}\%}} & {D121_Square_AVGQ} & {R32_Square_AVGQ} & {R64_Square_AVGQ} & {D121_Square_MEDQ} & {R32_Square_MEDQ} & {R64_Square_MEDQ} \\\\
NO SWITCH & {D121_NO_SWITCH_ASR}\%/\\textcolor{{magenta}}{{{D121_NO_SWITCH_ASR_small}\%}} & {R32_NO_SWITCH_ASR}\%/\\textcolor{{magenta}}{{{R32_NO_SWITCH_ASR_small}\%}} & {R64_NO_SWITCH_ASR}\%/\\textcolor{{magenta}}{{{R64_NO_SWITCH_ASR_small}\%}} & {D121_NO_SWITCH_AVGQ} & {R32_NO_SWITCH_AVGQ} & {R64_NO_SWITCH_AVGQ} & {D121_NO_SWITCH_MEDQ} & {R32_NO_SWITCH_MEDQ} & {R64_NO_SWITCH_MEDQ} \\\\
SWITCH$_\\text{{neg}}$ & {D121_SWITCH_neg_ASR}\%/\\textcolor{{magenta}}{{{D121_SWITCH_neg_ASR_small}\%}} & {R32_SWITCH_neg_ASR}\%/\\textcolor{{magenta}}{{{R32_SWITCH_neg_ASR_small}\%}} & {R64_SWITCH_neg_ASR}\%/\\textcolor{{magenta}}{{{R64_SWITCH_neg_ASR_small}\%}} & {D121_SWITCH_neg_AVGQ} & {R32_SWITCH_neg_AVGQ} & {R64_SWITCH_neg_AVGQ} & {D121_SWITCH_neg_MEDQ} & {R32_SWITCH_neg_MEDQ} & {R64_SWITCH_neg_MEDQ} \\\\
NO SWITCH$_\\text{{rnd}}$ & {D121_NO_SWITCH_rnd_ASR}\%/\\textcolor{{magenta}}{{{D121_NO_SWITCH_rnd_ASR_small}\%}} & {R32_NO_SWITCH_rnd_ASR}\%/\\textcolor{{magenta}}{{{R32_NO_SWITCH_rnd_ASR_small}\%}} & {R64_NO_SWITCH_rnd_ASR}\%/\\textcolor{{magenta}}{{{R64_NO_SWITCH_rnd_ASR_small}\%}} & {D121_NO_SWITCH_rnd_AVGQ} & {R32_NO_SWITCH_rnd_AVGQ} & {R64_NO_SWITCH_rnd_AVGQ} & {D121_NO_SWITCH_rnd_MEDQ} & {R32_NO_SWITCH_rnd_MEDQ} & {R64_NO_SWITCH_rnd_MEDQ} \\\\
SWITCH$_\\text{{other}}$ & {D121_SWITCH_other_ASR}\%/\\textcolor{{magenta}}{{{D121_SWITCH_other_ASR_small}\%}} & {R32_SWITCH_other_ASR}\%/\\textcolor{{magenta}}{{{R32_SWITCH_other_ASR_small}\%}} & {R64_SWITCH_other_ASR}\%/\\textcolor{{magenta}}{{{R64_SWITCH_other_ASR_small}\%}} & {D121_SWITCH_other_AVGQ} & {R32_SWITCH_other_AVGQ} & {R64_SWITCH_other_AVGQ} & {D121_SWITCH_other_MEDQ} & {R32_SWITCH_other_MEDQ} & {R64_SWITCH_other_MEDQ} \\\\
        """.format(
            D121_RGF_ASR=result["densenet121"]["RGF"]["success_rate"],R32_RGF_ASR=result["resnext32_4"]["RGF"]["success_rate"],R64_RGF_ASR=result["resnext64_4"]["RGF"]["success_rate"],
            D121_RGF_AVGQ=result["densenet121"]["RGF"][avg_q], R32_RGF_AVGQ=result["resnext32_4"]["RGF"][avg_q], R64_RGF_AVGQ=result["resnext64_4"]["RGF"][avg_q],
            D121_RGF_MEDQ=result["densenet121"]["RGF"][med_q], R32_RGF_MEDQ=result["resnext32_4"]["RGF"][med_q], R64_RGF_MEDQ=result["resnext64_4"]["RGF"][med_q],

            D121_RGF_ASR_small=result_2["densenet121"]["RGF"]["success_rate"],
            R32_RGF_ASR_small=result_2["resnext32_4"]["RGF"]["success_rate"],
            R64_RGF_ASR_small=result_2["resnext64_4"]["RGF"]["success_rate"],
            ...
            D121_PRGF_ASR_small=result_2["densenet121"]["PRGF"]["success_rate"],
            R32_PRGF_ASR_small=result_2["resnext32_4"]["PRGF"]["success_rate"],
            R64_PRGF_ASR_small=result_2["resnext64_4"]["PRGF"]["success_rate"],
            ...
            D121_Bandits_ASR_small=result_2["densenet121"]["Bandits"]["success_rate"],
            R32_Bandits_ASR_small=result_2["resnext32_4"]["Bandits"]["success_rate"],
            R64_Bandits_ASR_small=result_2["resnext64_4"]["Bandits"]["success_rate"],
            ...
            D121_PPBA_ASR_small=result_2["densenet121"]["PPBA"]["success_rate"],
            R32_PPBA_ASR_small=result_2["resnext32_4"]["PPBA"]["success_rate"],
            R64_PPBA_ASR_small=result_2["resnext64_4"]["PPBA"]["success_rate"],
            ...
            D121_Parsimonious_ASR_small=result_2["densenet121"]["Parsimonious"]["success_rate"],
            R32_Parsimonious_ASR_small=result_2["resnext32_4"]["Parsimonious"]["success_rate"],
            R64_Parsimonious_ASR_small=result_2["resnext64_4"]["Parsimonious"]["success_rate"],
            ...
            R32_SignHunter_MEDQ=result["resnext32_4"]["SignHunter"][med_q],
            R64_SignHunter_MEDQ=result["resnext64_4"]["SignHunter"][med_q],
            D121_SignHunter_ASR_small=result_2["densenet121"]["SignHunter"]["success_rate"],
            R32_SignHunter_ASR_small=result_2["resnext32_4"]["SignHunter"]["success_rate"],
            R64_SignHunter_ASR_small=result_2["resnext64_4"]["SignHunter"]["success_rate"],
            ...
            R32_Square_MEDQ=result["resnext32_4"]["Square"][med_q],
            R64_Square_MEDQ=result["resnext64_4"]["Square"][med_q],
            D121_Square_ASR_small=result_2["densenet121"]["Square"]["success_rate"],
            R32_Square_ASR_small=result_2["resnext32_4"]["Square"]["success_rate"],
            R64_Square_ASR_small=result_2["resnext64_4"]["Square"]["success_rate"],
            ...
            R32_NO_SWITCH_MEDQ=result["resnext32_4"]["NO_SWITCH"][med_q],
            R64_NO_SWITCH_MEDQ=result["resnext64_4"]["NO_SWITCH"][med_q],
            D121_NO_SWITCH_ASR_small=result_2["densenet121"]["NO_SWITCH"]["success_rate"],
            R32_NO_SWITCH_ASR_small=result_2["resnext32_4"]["NO_SWITCH"]["success_rate"],
            R64_NO_SWITCH_ASR_small=result_2["resnext64_4"]["NO_SWITCH"]["success_rate"],
            ...
            R32_SWITCH_neg_MEDQ=result["resnext32_4"]["SWITCH_neg"][med_q],
            R64_SWITCH_neg_MEDQ=result["resnext64_4"]["SWITCH_neg"][med_q],
            D121_SWITCH_neg_ASR_small=result_2["densenet121"]["SWITCH_neg"]["success_rate"],
            R32_SWITCH_neg_ASR_small=result_2["resnext32_4"]["SWITCH_neg"]["success_rate"],
            R64_SWITCH_neg_ASR_small=result_2["resnext64_4"]["SWITCH_neg"]["success_rate"],
            ...
            D121_NO_SWITCH_rnd_ASR_small=result_2["densenet121"]["NO_SWITCH_rnd"]["success_rate"],
            R32_NO_SWITCH_rnd_ASR_small=result_2["resnext32_4"]["NO_SWITCH_rnd"]["success_rate"],
            R64_NO_SWITCH_rnd_ASR_small=result_2["resnext64_4"]["NO_SWITCH_rnd"]["success_rate"],
            ...
            D121_SWITCH_other_ASR_small=result_2["densenet121"]["SWITCH_other"]["success_rate"],
            R32_SWITCH_other_ASR_small=result_2["resnext32_4"]["SWITCH_other"]["success_rate"],
            R64_SWITCH_other_ASR_small=result_2["resnext64_4"]["SWITCH_other"]["success_rate"],
        )
              )
    else:
        print("""
...
def draw_tables_for_CIFAR(norm, archs_result):
    ...
    if norm == "linf":
        print("""
                ...
                & & PPBA \cite{{li2020projection}} \cite{{li2020projection}} & {pyramidnet272_PPBA_ASR}\% & {gdas_PPBA_ASR}\% & {WRN28_PPBA_ASR}\% & {WRN40_PPBA_ASR}\% & {pyramidnet272_PPBA_AVGQ} & {gdas_PPBA_AVGQ} & {WRN28_PPBA_AVGQ} & {WRN40_PPBA_AVGQ} & {pyramidnet272_PPBA_MEDQ} & {gdas_PPBA_MEDQ} & {WRN28_PPBA_MEDQ} & {WRN40_PPBA_MEDQ} \\\\
                ...
                & & SWITCH & {pyramidnet272_SWITCH_neg_ASR}\% & {gdas_SWITCH_neg_ASR}\% & {WRN28_SWITCH_neg_ASR}\% & {WRN40_SWITCH_neg_ASR}\% & {pyramidnet272_SWITCH_neg_AVGQ} & {gdas_SWITCH_neg_AVGQ} & {WRN28_SWITCH_neg_AVGQ} & {WRN40_SWITCH_neg_AVGQ} & {pyramidnet272_SWITCH_neg_MEDQ} & {gdas_SWITCH_neg_MEDQ} & {WRN28_SWITCH_neg_MEDQ} & {WRN40_SWITCH_neg_MEDQ} \\\\
                & & SWITCH$_\\text{{RGF}}$ & {pyramidnet272_SWITCH_RGF_ASR}\% & {gdas_SWITCH_RGF_ASR}\% & {WRN28_SWITCH_RGF_ASR}\% & {WRN40_SWITCH_RGF_ASR}\% & {pyramidnet272_SWITCH_RGF_AVGQ} & {gdas_SWITCH_RGF_AVGQ} & {WRN28_SWITCH_RGF_AVGQ} & {WRN40_SWITCH_RGF_AVGQ} & {pyramidnet272_SWITCH_RGF_MEDQ} & {gdas_SWITCH_RGF_MEDQ} & {WRN28_SWITCH_RGF_MEDQ} & {WRN40_SWITCH_RGF_MEDQ} \\\\
...
def draw_tables_for_CIFAR_median_avg_query(norm, archs_result):
    result = archs_result
    avg_q = "avg_query_over_all_samples"
    med_q = "median_query_over_all_samples"
    if norm == "linf":
        print("""
                & {norm_str} & RGF \cite{{2017RGF}} & {pyramidnet272_RGF_ASR}\% & {gdas_RGF_ASR}\% & {WRN28_RGF_ASR}\% & {WRN40_RGF_ASR}\% & {pyramidnet272_RGF_MEDQ} & {gdas_RGF_MEDQ} & {WRN28_RGF_MEDQ} & {WRN40_RGF_MEDQ} & {pyramidnet272_RGF_AVGQ} & {gdas_RGF_AVGQ} & {WRN28_RGF_AVGQ} & {WRN40_RGF_AVGQ}  \\\\
                & & P-RGF \cite{{cheng2019improving}} & {pyramidnet272_PRGF_ASR}\% & {gdas_PRGF_ASR}\% & {WRN28_PRGF_ASR}\% & {WRN40_PRGF_ASR}\% & {pyramidnet272_PRGF_MEDQ} & {gdas_PRGF_MEDQ} & {WRN28_PRGF_MEDQ} & {WRN40_PRGF_MEDQ}  & {pyramidnet272_PRGF_AVGQ} & {gdas_PRGF_AVGQ} & {WRN28_PRGF_AVGQ} & {WRN40_PRGF_AVGQ} \\\\
                & & Bandits \cite{{ilyas2018prior}} & {pyramidnet272_Bandits_ASR}\% & {gdas_Bandits_ASR}\% & {WRN28_Bandits_ASR}\% & {WRN40_Bandits_ASR}\% & {pyramidnet272_Bandits_MEDQ} & {gdas_Bandits_MEDQ} & {WRN28_Bandits_MEDQ} & {WRN40_Bandits_MEDQ}  & {pyramidnet272_Bandits_AVGQ} & {gdas_Bandits_AVGQ} & {WRN28_Bandits_AVGQ} & {WRN40_Bandits_AVGQ}  \\\\
                & & PPBA \cite{{li2020projection}} \cite{{li2020projection}} & {pyramidnet272_PPBA_ASR}\% & {gdas_PPBA_ASR}\% & {WRN28_PPBA_ASR}\% & {WRN40_PPBA_ASR}\% & {pyramidnet272_PPBA_MEDQ} & {gdas_PPBA_MEDQ} & {WRN28_PPBA_MEDQ} & {WRN40_PPBA_MEDQ} & {pyramidnet272_PPBA_AVGQ} & {gdas_PPBA_AVGQ} & {WRN28_PPBA_AVGQ} & {WRN40_PPBA_AVGQ}  \\\\
                & & Parsimonious \cite{{moonICML19}} & {pyramidnet272_Parsimonious_ASR}\% & {gdas_Parsimonious_ASR}\% & {WRN28_Parsimonious_ASR}\% & {WRN40_Parsimonious_ASR}\% & {pyramidnet272_Parsimonious_MEDQ} & {gdas_Parsimonious_MEDQ} & {WRN28_Parsimonious_MEDQ} & {WRN40_Parsimonious_MEDQ} & {pyramidnet272_Parsimonious_AVGQ} & {gdas_Parsimonious_AVGQ} & {WRN28_Parsimonious_AVGQ} & {WRN40_Parsimonious_AVGQ} \\\\
                & & SignHunter \cite{{al2019sign}} & {pyramidnet272_SignHunter_ASR}\% & {gdas_SignHunter_ASR}\% & {WRN28_SignHunter_ASR}\% & {WRN40_SignHunter_ASR}\% & {pyramidnet272_SignHunter_MEDQ} & {gdas_SignHunter_MEDQ} & {WRN28_SignHunter_MEDQ} & {WRN40_SignHunter_MEDQ} & {pyramidnet272_SignHunter_AVGQ} & {gdas_SignHunter_AVGQ} & {WRN28_SignHunter_AVGQ} & {WRN40_SignHunter_AVGQ}  \\\\
                & & Square Attack \cite{{ACFH2020square}} & {pyramidnet272_Square_ASR}\% & {gdas_Square_ASR}\% & {WRN28_Square_ASR}\% & {WRN40_Square_ASR}\% & {pyramidnet272_Square_MEDQ} & {gdas_Square_MEDQ} & {WRN28_Square_MEDQ} & {WRN40_Square_MEDQ} & {pyramidnet272_Square_AVGQ} & {gdas_Square_AVGQ} & {WRN28_Square_AVGQ} & {WRN40_Square_AVGQ}  \\\\
                & & NO SWITCH & {pyramidnet272_NO_SWITCH_ASR}\% & {gdas_NO_SWITCH_ASR}\% & {WRN28_NO_SWITCH_ASR}\% & {WRN40_NO_SWITCH_ASR}\% & {pyramidnet272_NO_SWITCH_MEDQ} & {gdas_NO_SWITCH_MEDQ} & {WRN28_NO_SWITCH_MEDQ} & {WRN40_NO_SWITCH_MEDQ} & {pyramidnet272_NO_SWITCH_AVGQ} & {gdas_NO_SWITCH_AVGQ} & {WRN28_NO_SWITCH_AVGQ} & {WRN40_NO_SWITCH_AVGQ}  \\\\
                & & SWITCH & {pyramidnet272_SWITCH_neg_ASR}\% & {gdas_SWITCH_neg_ASR}\% & {WRN28_SWITCH_neg_ASR}\% & {WRN40_SWITCH_neg_ASR}\% & {pyramidnet272_SWITCH_neg_MEDQ} & {gdas_SWITCH_neg_MEDQ} & {WRN28_SWITCH_neg_MEDQ} & {WRN40_SWITCH_neg_MEDQ} & {pyramidnet272_SWITCH_neg_AVGQ} & {gdas_SWITCH_neg_AVGQ} & {WRN28_SWITCH_neg_AVGQ} & {WRN40_SWITCH_neg_AVGQ}  \\\\
                & & SWITCH$_\\text{{RGF}}$ & {pyramidnet272_SWITCH_RGF_ASR}\% & {gdas_SWITCH_RGF_ASR}\% & {WRN28_SWITCH_RGF_ASR}\% & {WRN40_SWITCH_RGF_ASR}\% & {pyramidnet272_SWITCH_RGF_MEDQ} & {gdas_SWITCH_RGF_MEDQ} & {WRN28_SWITCH_RGF_MEDQ} & {WRN40_SWITCH_RGF_MEDQ} & {pyramidnet272_SWITCH_RGF_AVGQ} & {gdas_SWITCH_RGF_AVGQ} & {WRN28_SWITCH_RGF_AVGQ} & {WRN40_SWITCH_RGF_AVGQ}  \\\\
        """.format(norm_str="$\ell_\infty$" if norm == "linf" else "$\ell_2$",
                   # pyramidnet272_NES_ASR=result["pyramidnet272"]["NES"]["success_rate"], gdas_NES_ASR=result["gdas"]["NES"]["success_rate"],
                   # WRN28_NES_ASR=result["WRN-28-10-drop"]["NES"]["success_rate"], WRN40_NES_ASR=result["WRN-40-10-drop"]["NES"]["success_rate"],
                   # pyramidnet272_NES_AVGQ=result["pyramidnet272"]["NES"][avg_q],
...
def draw_tables_for_CIFAR_with_slash(norm, result_small_queries, result_big_queries):
    result = result_big_queries
    result_2 = result_small_queries
    ...
    if norm == "linf":
        print("""
                & {norm_str} & RGF & {pyramidnet272_RGF_ASR}\%/\\textcolor{{magenta}}{{{pyramidnet272_RGF_ASR_small}\%}} & {gdas_RGF_ASR}\%/\\textcolor{{magenta}}{{{gdas_RGF_ASR_small}\%}} & {WRN28_RGF_ASR}\%/\\textcolor{{magenta}}{{{WRN28_RGF_ASR_small}\%}} & {WRN40_RGF_ASR}\%/\\textcolor{{magenta}}{{{WRN40_RGF_ASR_small}\%}} & {pyramidnet272_RGF_AVGQ} & {gdas_RGF_AVGQ} & {WRN28_RGF_AVGQ} & {WRN40_RGF_AVGQ} & {pyramidnet272_RGF_MEDQ} & {gdas_RGF_MEDQ} & {WRN28_RGF_MEDQ} & {WRN40_RGF_MEDQ} \\\\
                & & P-RGF & {pyramidnet272_PRGF_ASR}\%/\\textcolor{{magenta}}{{{pyramidnet272_PRGF_ASR_small}\%}} & {gdas_PRGF_ASR}\%/\\textcolor{{magenta}}{{{gdas_PRGF_ASR_small}\%}} & {WRN28_PRGF_ASR}\%/\\textcolor{{magenta}}{{{WRN28_PRGF_ASR_small}\%}} & {WRN40_PRGF_ASR}\%/\\textcolor{{magenta}}{{{WRN40_PRGF_ASR_small}\%}} & {pyramidnet272_PRGF_AVGQ} & {gdas_PRGF_AVGQ} & {WRN28_PRGF_AVGQ} & {WRN40_PRGF_AVGQ} & {pyramidnet272_PRGF_MEDQ} & {gdas_PRGF_MEDQ} & {WRN28_PRGF_MEDQ} & {WRN40_PRGF_MEDQ} \\\\
                & & Bandits & {pyramidnet272_Bandits_ASR}\%/\\textcolor{{magenta}}{{{pyramidnet272_Bandits_ASR_small}\%}} & {gdas_Bandits_ASR}\%/\\textcolor{{magenta}}{{{gdas_Bandits_ASR_small}\%}} & {WRN28_Bandits_ASR}\%/\\textcolor{{magenta}}{{{WRN28_Bandits_ASR_small}\%}} & {WRN40_Bandits_ASR}\%/\\textcolor{{magenta}}{{{WRN40_Bandits_ASR_small}\%}} & {pyramidnet272_Bandits_AVGQ} & {gdas_Bandits_AVGQ} & {WRN28_Bandits_AVGQ} & {WRN40_Bandits_AVGQ} & {pyramidnet272_Bandits_MEDQ} & {gdas_Bandits_MEDQ} & {WRN28_Bandits_MEDQ} & {WRN40_Bandits_MEDQ} \\\\
                & & PPBA & {pyramidnet272_PPBA_ASR}\%/\\textcolor{{magenta}}{{{pyramidnet272_PPBA_ASR_small}\%}} & {gdas_PPBA_ASR}\%/\\textcolor{{magenta}}{{{gdas_PPBA_ASR_small}\%}} & {WRN28_PPBA_ASR}\%/\\textcolor{{magenta}}{{{WRN28_PPBA_ASR_small}\%}} & {WRN40_PPBA_ASR}\%/\\textcolor{{magenta}}{{{WRN40_PPBA_ASR_small}\%}} & {pyramidnet272_PPBA_AVGQ} & {gdas_PPBA_AVGQ} & {WRN28_PPBA_AVGQ} & {WRN40_PPBA_AVGQ} & {pyramidnet272_PPBA_MEDQ} & {gdas_PPBA_MEDQ} & {WRN28_PPBA_MEDQ} & {WRN40_PPBA_MEDQ} \\\\
                & & Parsimonious & {pyramidnet272_Parsimonious_ASR}\%/\\textcolor{{magenta}}{{{pyramidnet272_Parsimonious_ASR_small}\%}} & {gdas_Parsimonious_ASR}\%/\\textcolor{{magenta}}{{{gdas_Parsimonious_ASR_small}\%}} & {WRN28_Parsimonious_ASR}\%/\\textcolor{{magenta}}{{{WRN28_Parsimonious_ASR_small}\%}} & {WRN40_Parsimonious_ASR}\%/\\textcolor{{magenta}}{{{WRN40_Parsimonious_ASR_small}\%}} & {pyramidnet272_Parsimonious_AVGQ} & {gdas_Parsimonious_AVGQ} & {WRN28_Parsimonious_AVGQ} & {WRN40_Parsimonious_AVGQ} & {pyramidnet272_Parsimonious_MEDQ} & {gdas_Parsimonious_MEDQ} & {WRN28_Parsimonious_MEDQ} & {WRN40_Parsimonious_MEDQ} \\\\
                & & SignHunter & {pyramidnet272_SignHunter_ASR}\%/\\textcolor{{magenta}}{{{pyramidnet272_SignHunter_ASR_small}\%}} & {gdas_SignHunter_ASR}\%/\\textcolor{{magenta}}{{{gdas_SignHunter_ASR_small}\%}} & {WRN28_SignHunter_ASR}\%/\\textcolor{{magenta}}{{{WRN28_SignHunter_ASR_small}\%}} & {WRN40_SignHunter_ASR}\%/\\textcolor{{magenta}}{{{WRN40_SignHunter_ASR_small}\%}} & {pyramidnet272_SignHunter_AVGQ} & {gdas_SignHunter_AVGQ} & {WRN28_SignHunter_AVGQ} & {WRN40_SignHunter_AVGQ} & {pyramidnet272_SignHunter_MEDQ} & {gdas_SignHunter_MEDQ} & {WRN28_SignHunter_MEDQ} & {WRN40_SignHunter_MEDQ} \\\\
                & & Square Attack & {pyramidnet272_Square_ASR}\%/\\textcolor{{magenta}}{{{pyramidnet272_Square_ASR_small}\%}} & {gdas_Square_ASR}\%/\\textcolor{{magenta}}{{{gdas_Square_ASR_small}\%}} & {WRN28_Square_ASR}\%/\\textcolor{{magenta}}{{{WRN28_Square_ASR_small}\%}} & {WRN40_Square_ASR}\%/\\textcolor{{magenta}}{{{WRN40_Square_ASR_small}\%}} & {pyramidnet272_Square_AVGQ} & {gdas_Square_AVGQ} & {WRN28_Square_AVGQ} & {WRN40_Square_AVGQ} & {pyramidnet272_Square_MEDQ} & {gdas_Square_MEDQ} & {WRN28_Square_MEDQ} & {WRN40_Square_MEDQ} \\\\
                & & NO SWITCH & {pyramidnet272_NO_SWITCH_ASR}\%/\\textcolor{{magenta}}{{{pyramidnet272_NO_SWITCH_ASR_small}\%}} & {gdas_NO_SWITCH_ASR}\%/\\textcolor{{magenta}}{{{gdas_NO_SWITCH_ASR_small}\%}} & {WRN28_NO_SWITCH_ASR}\%/\\textcolor{{magenta}}{{{WRN28_NO_SWITCH_ASR_small}\%}} & {WRN40_NO_SWITCH_ASR}\%/\\textcolor{{magenta}}{{{WRN40_NO_SWITCH_ASR_small}\%}} & {pyramidnet272_NO_SWITCH_AVGQ} & {gdas_NO_SWITCH_AVGQ} & {WRN28_NO_SWITCH_AVGQ} & {WRN40_NO_SWITCH_AVGQ} & {pyramidnet272_NO_SWITCH_MEDQ} & {gdas_NO_SWITCH_MEDQ} & {WRN28_NO_SWITCH_MEDQ} & {WRN40_NO_SWITCH_MEDQ} \\\\
                & & SWITCH$_\\text{{neg}}$ & {pyramidnet272_SWITCH_neg_ASR}\%/\\textcolor{{magenta}}{{{pyramidnet272_SWITCH_neg_ASR_small}\%}} & {gdas_SWITCH_neg_ASR}\%/\\textcolor{{magenta}}{{{gdas_SWITCH_neg_ASR_small}\%}} & {WRN28_SWITCH_neg_ASR}\%/\\textcolor{{magenta}}{{{WRN28_SWITCH_neg_ASR_small}\%}} & {WRN40_SWITCH_neg_ASR}\%/\\textcolor{{magenta}}{{{WRN40_SWITCH_neg_ASR_small}\%}} & {pyramidnet272_SWITCH_neg_AVGQ} & {gdas_SWITCH_neg_AVGQ} & {WRN28_SWITCH_neg_AVGQ} & {WRN40_SWITCH_neg_AVGQ} & {pyramidnet272_SWITCH_neg_MEDQ} & {gdas_SWITCH_neg_MEDQ} & {WRN28_SWITCH_neg_MEDQ} & {WRN40_SWITCH_neg_MEDQ} \\\\
                & & NO SWITCH$_\\text{{rnd}}$ & {pyramidnet272_NO_SWITCH_rnd_ASR}\%/\\textcolor{{magenta}}{{{pyramidnet272_NO_SWITCH_rnd_ASR_small}\%}} & {gdas_NO_SWITCH_rnd_ASR}\%/\\textcolor{{magenta}}{{{gdas_NO_SWITCH_rnd_ASR_small}\%}} & {WRN28_NO_SWITCH_rnd_ASR}\%/\\textcolor{{magenta}}{{{WRN28_NO_SWITCH_rnd_ASR_small}\%}} & {WRN40_NO_SWITCH_rnd_ASR}\%/\\textcolor{{magenta}}{{{WRN40_NO_SWITCH_rnd_ASR_small}\%}} & {pyramidnet272_NO_SWITCH_rnd_AVGQ} & {gdas_NO_SWITCH_rnd_AVGQ} & {WRN28_NO_SWITCH_rnd_AVGQ} & {WRN40_NO_SWITCH_rnd_AVGQ} & {pyramidnet272_NO_SWITCH_rnd_MEDQ} & {gdas_NO_SWITCH_rnd_MEDQ} & {WRN28_NO_SWITCH_rnd_MEDQ} & {WRN40_NO_SWITCH_rnd_MEDQ} \\\\
                & & SWITCH$_\\text{{other}}$ & {pyramidnet272_SWITCH_other_ASR}\%/\\textcolor{{magenta}}{{{pyramidnet272_SWITCH_other_ASR_small}\%}} & {gdas_SWITCH_other_ASR}\%/\\textcolor{{magenta}}{{{gdas_SWITCH_other_ASR_small}\%}} & {WRN28_SWITCH_other_ASR}\%/\\textcolor{{magenta}}{{{WRN28_SWITCH_other_ASR_small}\%}} & {WRN40_SWITCH_other_ASR}\%/\\textcolor{{magenta}}{{{WRN40_SWITCH_other_ASR_small}\%}} & {pyramidnet272_SWITCH_other_AVGQ} & {gdas_SWITCH_other_AVGQ} & {WRN28_SWITCH_other_AVGQ} & {WRN40_SWITCH_other_AVGQ} & {pyramidnet272_SWITCH_other_MEDQ} & {gdas_SWITCH_other_MEDQ} & {WRN28_SWITCH_other_MEDQ} & {WRN40_SWITCH_other_MEDQ} \\\\
        """.format(norm_str="$\ell_\infty$" if norm == "linf" else "$\ell_2$",
                   pyramidnet272_RGF_ASR=result["pyramidnet272"]["RGF"]["success_rate"],
                   gdas_RGF_ASR=result["gdas"]["RGF"]["success_rate"],
                   WRN28_RGF_ASR=result["WRN-28-10-drop"]["RGF"]["success_rate"],
                   ...
                   pyramidnet272_RGF_ASR_small=result_2["pyramidnet272"]["RGF"]["success_rate"],
                   gdas_RGF_ASR_small=result_2["gdas"]["RGF"]["success_rate"],
                   WRN28_RGF_ASR_small=result_2["WRN-28-10-drop"]["RGF"]["success_rate"],
                   WRN40_RGF_ASR_small=result_2["WRN-40-10-drop"]["RGF"]["success_rate"],
                   ...
                   pyramidnet272_PRGF_ASR_small=result_2["pyramidnet272"]["PRGF"]["success_rate"],
                   gdas_PRGF_ASR_small=result_2["gdas"]["PRGF"]["success_rate"],
                   WRN28_PRGF_ASR_small=result_2["WRN-28-10-drop"]["PRGF"]["success_rate"],
                   WRN40_PRGF_ASR_small=result_2["WRN-40-10-drop"]["PRGF"]["success_rate"],
                   ...
                   pyramidnet272_Bandits_ASR_small=result_2["pyramidnet272"]["Bandits"]["success_rate"],
                   gdas_Bandits_ASR_small=result_2["gdas"]["Bandits"]["success_rate"],
                   WRN28_Bandits_ASR_small=result_2["WRN-28-10-drop"]["Bandits"]["success_rate"],
                   WRN40_Bandits_ASR_small=result_2["WRN-40-10-drop"]["Bandits"]["success_rate"],
                   ...
                   pyramidnet272_PPBA_ASR_small=result_2["pyramidnet272"]["PPBA"]["success_rate"],
                   gdas_PPBA_ASR_small=result_2["gdas"]["PPBA"]["success_rate"],
                   WRN28_PPBA_ASR_small=result_2["WRN-28-10-drop"]["PPBA"]["success_rate"],
                   WRN40_PPBA_ASR_small=result_2["WRN-40-10-drop"]["PPBA"]["success_rate"],
                   ...
                   pyramidnet272_Parsimonious_ASR_small=result_2["pyramidnet272"]["Parsimonious"]["success_rate"],
                   gdas_Parsimonious_ASR_small=result_2["gdas"]["Parsimonious"]["success_rate"],
                   WRN28_Parsimonious_ASR_small=result_2["WRN-28-10-drop"]["Parsimonious"]["success_rate"],
                   WRN40_Parsimonious_ASR_small=result_2["WRN-40-10-drop"]["Parsimonious"]["success_rate"],
                   # pyramidnet272_SimBA_ASR=result["pyramidnet272"]["SimBA"]["success_rate"],
                   # gdas_SimBA_ASR=result["gdas"]["SimBA"]["success_rate"],
                   # WRN28_SimBA_ASR=result["WRN-28-10-drop"]["SimBA"]["success_rate"],
                   # WRN40_SimBA_ASR=result["WRN-40-10-drop"]["SimBA"]["success_rate"],
                   ...
                   pyramidnet272_SignHunter_ASR_small=result_2["pyramidnet272"]["SignHunter"]["success_rate"],
                   gdas_SignHunter_ASR_small=result_2["gdas"]["SignHunter"]["success_rate"],
                   WRN28_SignHunter_ASR_small=result_2["WRN-28-10-drop"]["SignHunter"]["success_rate"],
                   WRN40_SignHunter_ASR_small=result_2["WRN-40-10-drop"]["SignHunter"]["success_rate"],
                   ...
                   pyramidnet272_Square_ASR_small=result_2["pyramidnet272"]["Square"]["success_rate"],
                   gdas_Square_ASR_small=result_2["gdas"]["Square"]["success_rate"],
                   WRN28_Square_ASR_small=result_2["WRN-28-10-drop"]["Square"]["success_rate"],
                   WRN40_Square_ASR_small=result_2["WRN-40-10-drop"]["Square"]["success_rate"],
                   ...
                   pyramidnet272_NO_SWITCH_ASR_small=result_2["pyramidnet272"]["NO_SWITCH"]["success_rate"],
                   gdas_NO_SWITCH_ASR_small=result_2["gdas"]["NO_SWITCH"]["success_rate"],
                   WRN28_NO_SWITCH_ASR_small=result_2["WRN-28-10-drop"]["NO_SWITCH"]["success_rate"],
                   WRN40_NO_SWITCH_ASR_small=result_2["WRN-40-10-drop"]["NO_SWITCH"]["success_rate"],
                   ...
                   pyramidnet272_SWITCH_neg_ASR_small=result_2["pyramidnet272"]["SWITCH_neg"]["success_rate"],
                   gdas_SWITCH_neg_ASR_small=result_2["gdas"]["SWITCH_neg"]["success_rate"],
                   WRN28_SWITCH_neg_ASR_small=result_2["WRN-28-10-drop"]["SWITCH_neg"]["success_rate"],
                   WRN40_SWITCH_neg_ASR_small=result_2["WRN-40-10-drop"]["SWITCH_neg"]["success_rate"],
                   ...
                   pyramidnet272_NO_SWITCH_rnd_ASR_small=result_2["pyramidnet272"]["NO_SWITCH_rnd"]["success_rate"],
                   gdas_NO_SWITCH_rnd_ASR_small=result_2["gdas"]["NO_SWITCH_rnd"]["success_rate"],
                   WRN28_NO_SWITCH_rnd_ASR_small=result_2["WRN-28-10-drop"]["NO_SWITCH_rnd"]["success_rate"],
                   WRN40_NO_SWITCH_rnd_ASR_small=result_2["WRN-40-10-drop"]["NO_SWITCH_rnd"]["success_rate"],
                   ...
                   pyramidnet272_SWITCH_other_ASR_small=result_2["pyramidnet272"]["SWITCH_other"]["success_rate"],
                   gdas_SWITCH_other_ASR_small=result_2["gdas"]["SWITCH_other"]["success_rate"],
                   WRN28_SWITCH_other_ASR_small=result_2["WRN-28-10-drop"]["SWITCH_other"]["success_rate"],
                   WRN40_SWITCH_other_ASR_small=result_2["WRN-40-10-drop"]["SWITCH_other"]["success_rate"],
                   )
              )
    else:
        print("""
                ...
                """.format(norm_str="$\ell_\infty$" if norm == "linf" else "$\ell_2$",
                           ...
                           pyramidnet272_NO_SWITCH_rnd_ASR_small=result_2["pyramidnet272"]["NO_SWITCH_rnd"][
                               "success_rate"],
                           gdas_NO_SWITCH_rnd_ASR_small=result_2["gdas"]["NO_SWITCH_rnd"]["success_rate"],
                           WRN28_NO_SWITCH_rnd_ASR_small=result_2["WRN-28-10-drop"]["NO_SWITCH_rnd"]["success_rate"],
                           WRN40_NO_SWITCH_rnd_ASR_small=result_2["WRN-40-10-drop"]["NO_SWITCH_rnd"]["success_rate"],
                           ...
                           pyramidnet272_SWITCH_other_ASR_small=result_2["pyramidnet272"]["SWITCH_other"][
                               "success_rate"],
                           gdas_SWITCH_other_ASR_small=result_2["gdas"]["SWITCH_other"]["success_rate"],
                           WRN28_SWITCH_other_ASR_small=result_2["WRN-28-10-drop"]["SWITCH_other"]["success_rate"],
                           WRN40_SWITCH_other_ASR_small=result_2["WRN-40-10-drop"]["SWITCH_other"]["success_rate"],
...
if __name__ == "__main__":
    ...
    for arch in archs:
        result = fetch_all_json_content_given_contraint(dataset, norm, targeted, arch, limited_queries_small)
        result_archs_small_queries[arch] = result
    if "CIFAR" in dataset:
        draw_tables_for_CIFAR(norm,  result_archs_small_queries)
    elif "TinyImageNet" in dataset:
        draw_tables_for_TinyImageNet(norm, result_archs_small_queries)
