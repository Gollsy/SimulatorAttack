    ...
    for (dataset, norm, targeted, method), dir_path in dataset_path_dict.items():
        for file_path in os.listdir(dir_path):
            if arch in file_path and file_path.endswith(".json") and not file_path.startswith("tmp"):
                if "meta_interval" in file_path:
                    if not file_path.endswith("meta_interval_5_result.json"):
                        print("Filter out {}".format(file_path))
                        continue
...
method_name_to_paper = {"bandits_attack":"Bandits", "P-RGF_biased_attack":"P-RGF","P-RGF_uniform_attack":"RGF",
                        "MetaGradAttack":"Meta Attack", "NES-attack":"NES",
...
def get_all_exists_folder(dataset, methods, norm, targeted):
    root_dir = "/mnt/workspace/SimulatorAttack/logs/"
...
if __name__ == "__main__":
    args = parse_args()
    dump_folder = "/mnt/workspace/SimulatorAttack/figures_without_font_embed/{}/".format(args.fig_type)
    os.makedirs(dump_folder, exist_ok=True)
